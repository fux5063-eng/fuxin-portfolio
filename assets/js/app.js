...
(function () {
  ...
  function staticPanel(m, opt = {}) {
    ...
    const imgs = (st.images || []).map(it => `
          <figure class="m3di__item">
            <img src="${esc(it.f)}"${vattrs(it.f, '(max-width:1000px) 100vw, 47vw')}${bgc(it.f)}${dim(it.f)} alt="${esc(it.alt || '')}" loading="lazy" decoding="async">
    ...
    return `
      <figure class="cmpwrap">
        <div class="cmp" data-cmp>
          <img src="${esc(c.before.f)}"${vattrs(c.before.f, '(max-width:1000px) 100vw, 47vw')}${bgc(c.before.f)}${dim(c.before.f)} alt="${esc(c.before.label || '过程')}" loading="lazy" decoding="async">
          <div class="cmp__after"><img src="${esc(c.after.f)}"${vattrs(c.after.f, '(max-width:1000px) 100vw, 47vw')}${bgc(c.after.f)}${dim(c.after.f)} alt="${esc(c.after.label || '成品')}" loading="lazy" decoding="async"></div>
        ...
        <div class="tabs__panes">
          ${list.map((x, i) => `
            <div class="tabs__pane${i === 0 ? ' on' : ''}" data-i="${i}" role="tabpanel">
              <figure class="shot"><img src="${esc(x.f)}"${vattrs(x.f, '(max-width:1000px) 100vw, 47vw')}${bgc(x.f)}${dim(x.f)} alt="${esc(x.t)}" loading="lazy" decoding="async"></figure>
  ...
  function viewHome() {
    ...
    const pickCard = (key, idx) => {
      ...
      <a class="pick rv" href="#${dirId}/${slug}">
        ...
        <div class="pick__img"><img src="${esc(cover)}"${vattrs(cover, '(max-width:700px) 92vw, (max-width:1100px) 46vw, 33vw')}${bgc(cover)}${dim(cover)} alt="${esc(p.title)}" loading="${idx < 2 ? 'eager' : 'lazy'}" decoding="async"></div>
    ...
    <div class="sheet">
      <section class="sec" id="aboutTeaser">
        <div class="wrap">
          <div class="about">
            <div class="about__pic rv"><img src="assets/img/about/portrait.webp"${vattrs('assets/img/about/portrait.webp', '(max-width:1000px) 92vw, 360px')}${bgc('assets/img/about/portrait.webp')}${dim('assets/img/about/portrait.webp')} alt="${esc(SITE.name)}" loading="lazy"></div>
            <div class="rv">
  ...
  function viewDirection(d) {
    ...
    const cards = d.projects.map(p => `
      <a class="card rv" data-slug="${p.slug}" href="${p.link || "#" + d.id + "/" + p.slug}" data-tags="${esc((p.tags || []).join('|'))}">
        <canvas class="card__fx" aria-hidden="true"></canvas>
        <div class="card__img"><img src="${coverOf(p, d)}"${vattrs(coverOf(p, d), '(max-width:640px) 92vw, (max-width:1000px) 46vw, 21vw')}${bgc(coverOf(p, d))}${dim(coverOf(p, d))} alt="${esc(p.title)}" loading="lazy" decoding="async"></div>
  ...
  function viewProject(d, p) {
    ...
    const fig = (f, cls, sizes) => `
      <figure class="fig ${f.wide ? 'fig--wide' : ''} ${cls || ''}">
        <img src="${f.f}"${vattrs(f.f, sizes)}${bgc(f.f)}${dim(f.f)} alt="${esc(f.cap || p.title)}" loading="lazy" decoding="async">
    ...
    const hero = p.hero ? `
      <figure class="heroFig shot">
        <img src="${p.hero.f}"${vattrs(p.hero.f, '(max-width:1000px) 100vw, 1180px')}${bgc(p.hero.f)}${dim(p.hero.f)} alt="${esc(p.hero.cap || p.title)}" loading="eager" fetchpriority="high" decoding="async">
  ...
  function viewAbout() {
    ...
    <section class="sec sec--v2">
      ...
      <div class="wrap">
      <div class="about">
        <figure class="about__pic shot rv"><img src="assets/img/about/portrait.webp"${vattrs('assets/img/about/portrait.webp', '(max-width:1000px) 92vw, 360px')}${bgc('assets/img/about/portrait.webp')}${dim('assets/img/about/portrait.webp')} alt="${esc(SITE.name)}" loading="lazy"></figure>
        <div>
          <p style="color:var(--ink-2)">${esc(ABOUT.intro)}</p>
...
function bgc(p) {                          /* 加载前的占位底色 */
  var c = (typeof IMGC !== 'undefined' && IMGC[p]) || '';
  return c ? ' style="background:' + c + '"' : '';
}
function dim(p) {                          /* 预留位置：避免图片加载时页面跳动 */
  var d = (typeof IMD !== 'undefined' && IMD[p]) || null;
